#include "game.h"
#include "player.h"
#include "fantasma.h" // Inclua fantasma.h para fantasmas_init/update/draw/check_collision e MAX_FANTASMAS
#include "paredes.h"
#include "item.h"     // Inclui o modulo de itens
...
#include <allegro5/allegro_audio.h>   // Essencial para audio
#include <allegro5/allegro_acodec.h>  // Essencial para carregar .ogg, .wav etc.
#include <stdio.h>
#include <stdlib.h>
#include <math.h>

// Variáveis globais acessíveis externamente (declaradas em game.h como extern)
float mouse_x = LARGURA_TELA / 2.0f;
float mouse_y = ALTURA_TELA / 2.0f;
float camera_x = 0;
float camera_y = 0;
float game_time = 0.0f;           // Tempo de jogo (em segundos)
bool escape_area_active = false;  // Inicialmente inativa

GAME_STATE current_game_state = GAME_STATE_MENU; // Inicia no estado de menu

// Variáveis de áudio globais (atribuição de NULL aqui)
ALLEGRO_SAMPLE *trilha_sonora = NULL;
ALLEGRO_SAMPLE_INSTANCE *inst_trilha_sonora = NULL;
ALLEGRO_SAMPLE *fantasma_morrendo_sfx = NULL; // <-- VARIÁVEL AQUI

// Variáveis estáticas internas ao modulo game.c
static ALLEGRO_DISPLAY *display = NULL;
static ALLEGRO_EVENT_QUEUE *event_queue = NULL;
static ALLEGRO_TIMER *timer = NULL;

static ALLEGRO_FONT *font_big_game_over = NULL;   // Usada para Game Over/Vitoria
static ALLEGRO_FONT *font_small_hud = NULL;      // Usada para HUD e tutorial

static ALLEGRO_FONT *font_title_menu = NULL;     // Nova fonte para o titulo do menu
static ALLEGRO_FONT *font_button_menu = NULL;    // Nova fonte para botoes do menu
static ALLEGRO_FONT *font_tutorial_menu = NULL;  // Nova fonte para o tutorial do menu

static bool keys[NUM_KEYS] = {false};
static bool done_main_loop = false; // Flag para sair do loop principal no main.c
static bool redraw = true;           // Flag para controlar o redesenho da tela
static bool game_over = false;       // Indica se o jogo terminou (seja por derrota ou vitoria)
static bool game_won = false;        // Nova flag para indicar vitoria
static float opacidade_game_over = 0.0f;

static Player player; // Instancia do jogador
...
// Novas variaveis para a posicao da area de fuga
static float escape_x, escape_y;
static const float ESCAPE_AREA_SIZE = 32.0f; // Tamanho da area de fuga (ex: igual ao player)

// Funções auxiliares (static para serem internas a este arquivo)
...
// Essa funcao desenha os feixes de luz amarelos transparentes
static void desenhar_feixes_visuais() {
    ...
    for (int i = 0; i <= num_feixes; i++) {
        ...
        // Desenha o feixe visual visivel
...
// Essa funcao cria o efeito de "furo" na sombra do ambiente, simulando a luz da lanterna
static void draw_lanterna() {
    ...
    // Define o modo de blend para "furar" a sombra
    ...
    float light_angle_spread = ALLEGRO_PI / 4.0f;
    float light_length = 150.0f;
    int num_feixes = 200; // Maior numero de feixes para um circulo mais suave
    ...
    for (int i = 0; i <= num_feixes; i++) {
        ...
        al_draw_line(light_origin_x - camera_x, light_origin_y - camera_y,
                     end_x - camera_x, end_y - camera_y,
                     al_map_rgba(255, 255, 255, 255), 2); // Cor branca para "furar"
    }

    // Restaura para desenhar na tela principal e aplica a sombra
...
// Funcao para desenhar a tela de Game Over ou Vitoria
...
void reset_game() {
    player_init(&player);
    player_posicionar_no_ponto_p(&player); // Posicionar no ponto 'P' do mapa

    // Captura a posicao do ponto P para a area de fuga
    escape_x = player.x;
    escape_y = player.y;

    fantasmas_init();
    paredes_init();
    itens_reset(); // Reseta os itens para um novo jogo e gera o primeiro
    game_time = 0.0f; // Reseta o tempo de jogo

    lanterna_ativa = false; // Garante que a lanterna comece desativada
    tempo_lanterna = 0.0f;
    cooldown_lanterna = 0.0f;

    escape_area_active = false; // Garante que a area de fuga comece inativa

    for (int i = 0; i < NUM_KEYS; i++)
        keys[i] = false;

    game_over = false;
    game_won = false; // Reseta a flag de vitoria
    opacidade_game_over = 0.0f;

    // Ajuste inicial da câmera para centralizar o jogador, respeitando limites do mapa
    update_camera(); // Chama para garantir que a camera esteja no lugar certo ao reiniciar

    // Reinicia a musica de fundo ao reiniciar o jogo
...
void game_init() {
    ...
    al_init_font_addon();
    al_init_ttf_addon();
    al_install_audio();  // Essencial para audio
    al_init_acodec_addon(); // Essencial para carregar .ogg, .wav etc.
    al_reserve_samples(10); // Aumentado para 10 canais
    ...
    al_set_window_title(display, "Jogo de Horror com Lanterna"); // Titulo da janela
    ...
    // Carregar fontes para o jogo e HUD
    font_big_game_over = al_load_ttf_font("fonts/OpenSans-Regular.ttf", 48, 0); // Fonte para GAME OVER
    if (!font_big_game_over) {
        fprintf(stderr, "Erro ao carregar font_big_game_over. Verifique 'fonts/OpenSans-Regular.ttf'.\n");
        // Fallback: criar uma fonte embutida se a principal falhar
        font_big_game_over = al_create_builtin_font();
    }

    font_small_hud = al_load_ttf_font("fonts/OpenSans-Regular.ttf", 20, 0); // Fonte para HUD e mensagens pequenas
    if (!font_small_hud) {
        fprintf(stderr, "Erro ao carregar font_small_hud. Verifique 'fonts/OpenSans-Regular.ttf'.\n");
        font_small_hud = al_create_builtin_font();
    }

    // Carregar fontes para o menu (OpenSans-Regular.ttf)
    font_title_menu = al_load_ttf_font("fonts/OpenSans-Regular.ttf", 72, 0); // Fonte maior para o titulo
    if (!font_title_menu) {
    ...
    font_button_menu = al_load_ttf_font("fonts/OpenSans-Regular.ttf", 36, 0); // Fonte para botoes
    if (!font_button_menu) {
    ...
    font_tutorial_menu = al_load_ttf_font("fonts/OpenSans-Regular.ttf", 20, 0); // Fonte para o tutorial
    ...
    // CARREGAMENTO DOS ARQUIVOS DE AUDIO (conforme sua sugestão)
    trilha_sonora = al_load_sample("audio/piano-terror.ogg"); // Caminho do arquivo de audio (na pasta 'audio')
    if (!trilha_sonora) {
        ...
        if (!inst_trilha_sonora) {
            ...
            al_set_sample_instance_gain(inst_trilha_sonora, 0.8f); // Volume ajustado para 0.8
        }
    }

    // NOVO: Carregar som de fantasma morrendo
    fantasma_morrendo_sfx = al_load_sample("audio/ghost-scream.ogg"); // <-- SUBSTITUÍDO PELO SEU NOME DE ARQUIVO
        ...
        switch (ev.type) {
            case ALLEGRO_EVENT_TIMER:
                ...
                if (current_game_state == GAME_STATE_PLAYING && !game_over) {
                    // A musica toca aqui se 'tocar_trilha_sonora' for true
                    // Isso sera controlado pelo clique no menu.
                    if (inst_trilha_sonora && !al_get_sample_instance_playing(inst_trilha_sonora)) {
                         al_play_sample_instance(inst_trilha_sonora);
                    }

                    game_time += 1.0f / FPS; // Incrementa o tempo de jogo a cada frame
                    player_update(&player, keys);
                    fantasmas_update(&player);
                    itens_update(&player);
                    update_camera();

                    // Verifica colisoes e condicao de vitoria/derrota
                    if (fantasmas_check_collision(&player)) {
                        game_over = true;
                        game_won = false; // Perdeu
                        if (inst_trilha_sonora) al_stop_sample_instance(inst_trilha_sonora); // Para a musica
                    }

                    // Logica da area de fuga
                    if (itens_get_coletados() >= MAX_ITENS) {
                        escape_area_active = true; // Ativa a area de fuga
                        // Verifica se o jogador colidiu com a area de fuga
                    ...
                    if (lanterna_ativa) {
                        ...
                        if (cooldown_lanterna > 0.0f) {
                            cooldown_lanterna -= 1.0f / FPS; // CORRIGIDO: Era cooldown_cooldown, agora cooldown_lanterna
                            if(cooldown_lanterna < 0.0f) {
                                cooldown_lanterna = 0.0f;
                            }
                        }
            ...
            case ALLEGRO_EVENT_MOUSE_BUTTON_DOWN:
                if (current_game_state == GAME_STATE_MENU) {
                    game_menu_update(&ev);
                    if (current_game_state == GAME_STATE_PLAYING) {
                        reset_game(); // Reseta o jogo para comecar
                    }
                }
                break;
        }
...
void game_destroy() {
    ...
    // DESTRUIR INSTANCIAS (conforme sua sugestão)
    if (inst_trilha_sonora) {
    ...
    // NOVO: Destruir o som de fantasma
    if (fantasma_morrendo_sfx) {
...
void game_menu_draw() {
    ...
    if (font_title_menu) {
        al_draw_text(font_title_menu, text_color, LARGURA_TELA / 2, ALTURA_TELA / 4,
                     ALLEGRO_ALIGN_CENTER, "Labirinto Sombrio");
    } else {
