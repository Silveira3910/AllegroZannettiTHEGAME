...
void player_update(Player *p, const bool keys[]) {
    ...
    // Checa colisão nas direções separadas (evita travamento em cantos)
...
void player_draw(const Player *p, float camera_x, float camera_y) {
    ...
    float raio = p->largura / 2.0f;

    al_draw_filled_circle(centro_x, centro_y, raio, al_map_rgb(0, 255, 0));
}
